...
func (j *Job) worker(id int) {
	...
	for {
		...
		case <-j.ctx.Done():
			...
			return
		case urlStr := <-j.pending:
			j.processURL(urlStr, id)
		}
	}
}

func (j *Job) processURL(urlStr string, workerID int) {
	depth := j.depths[urlStr]
	log.Printf("[Worker %d] Processing: %s (depth %d)", workerID, urlStr, depth)

	if depth > j.Config.MaxDepth {
		atomic.AddInt64(&j.stats.Skipped, 1)
		log.Printf("[Worker %d] Max depth reached for %s", workerID, urlStr)
		return
	}
	...
	if err != nil {
		log.Printf("[Worker %d] Download failed for %s: %v", workerID, urlStr, err)
	...
	if j.hashes[hash] {
		j.mu.Unlock()
		atomic.AddInt64(&j.stats.Skipped, 1)
		log.Printf("[Worker %d] Duplicate content for %s", workerID, urlStr)
		return
	}
	j.hashes[hash] = true
	j.mu.Unlock()
	...
	for _, handler := range j.sortedHandlers() {
		...
		if err != nil {
			log.Printf("[Worker %d] Handler error for %s: %v", workerID, urlStr, err)
		} else {
	...
	if err != nil {
		log.Printf("[Worker %d] Save failed for %s: %v", workerID, urlStr, err)
		atomic.AddInt64(&j.stats.Failed, 1)
		return
	}
	...
	log.Printf("[Worker %d] ✅ Saved: %s → %s", workerID, urlStr, savedPath)

	// Парсим ссылки для дальнейшего скачивания (используем оригинальный контент!)
	if depth < j.Config.MaxDepth {
		j.parseAndQueueLinks(content, contentType, urlStr, depth, workerID)
	}
}

func (j *Job) parseAndQueueLinks(content []byte, contentType, baseURL string, depth, workerID int) {
	for _, parser := range j.Parsers {
		if parser.CanParse(contentType) {
			rawLinks, err := parser.Parse(content, baseURL)
			if err != nil {
				log.Printf("[Worker %d] Parse error for %s: %v", workerID, baseURL, err)
				continue
			}

			log.Printf("[Worker %d] Found %d raw links in %s", workerID, len(rawLinks), baseURL)
			...
			for _, rawLink := range rawLinks {
				...
				if !j.Filter.ShouldDownload(normalized) {
					...
					log.Printf("[Worker %d] Filtered out: %s (%s)", workerID, normalized, reason)
					atomic.AddInt64(&j.stats.Skipped, 1)
					continue
				}
				...
				if !j.visited[normalized] {
					...
					case j.pending <- normalized:
						log.Printf("[Worker %d] Enqueued: %s (depth %d)", workerID, normalized, depth+1)
					default:
						log.Printf("[Worker %d] Queue full, dropping: %s", workerID, normalized)
					}
				}
				j.mu.Unlock()
			}
...
func (j *Job) progressReporter() {
	...
	for {
		...
		case <-ticker.C:
			...
			fmt.Printf("📊 Progress: Files=%d, Bytes=%.2fMB, Failed=%d, Pending=%d, Visited=%d, Speed=%.2fKB/s\n",
				j.stats.TotalFiles,
				float64(j.stats.DownloadedBytes)/(1024*1024),
				j.stats.Failed,
				pending,
				visited,
				speed/1024,
